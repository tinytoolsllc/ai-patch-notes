...
describe("HomePage", () => {
  describe("anonymous user", () => {
    ...
    it("renders AI summary as markdown with headings and lists", async () => {
      ...
      expect(await screen.findByText("Removed legacy context API")).toBeInTheDocument();
    });
    ...
    it("sees the hero card", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/Never miss a release that matters/)).toBeInTheDocument();
      });
    });
    ...
    it("does not see the empty watchlist prompt", async () => {
      ...
      expect(screen.queryByText(/Add packages to your watchlist/)).not.toBeInTheDocument();
    });
  });
  ...
  describe("authenticated user with watchlist", () => {
      ...
      server.use(
        ...
        http.get("/api/feed", () => {
          ...
          return HttpResponse.json({
            groups: mockFeedGroups.filter((g) => g.packageId === "pkg-react-test-id"),
          });
        }),
      );
    ...
    it("does not show hero card", async () => {
      ...
      expect(screen.queryByText(/Never miss a release that matters/)).not.toBeInTheDocument();
    });
  });
  ...
  describe("authenticated user with empty watchlist", () => {
    ...
    it("sees prompt to add packages", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/Add packages to your watchlist/)).toBeInTheDocument();
      });
    });
