...
describe("sendDigest", () => {
  ...
  it("sends digests to all users and logs summary on full success", async () => {
    ...
    expect(mockSend).toHaveBeenCalledWith(expect.objectContaining({ to: "Alice <a@test.com>" }));
    expect(mockSend).toHaveBeenCalledWith(expect.objectContaining({ to: "Bob <b@test.com>" }));
      ...
      packages: [
        {
          ...
          oldestVersion: "v1.0.0",
          summary: "Summary for v1.0.0",
        },
      ],
  ...
  it("skips user and records failure when template rendering fails", async () => {
    ...
    await expect(sendDigest(makeTimer(), context)).rejects.toThrow("Digest send partially failed");
    expect(mockSend).not.toHaveBeenCalled();
    expect(context.error).toHaveBeenCalledWith(
